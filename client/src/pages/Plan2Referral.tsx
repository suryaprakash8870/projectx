...
export default function Plan2Referral() {
  ...
  const isAdminInPlan2 = role === 'ADMIN' && adminPlan === 'PLAN2';
  const isPlan2User = planType === 'PLAN2';

  // Plan 2 user — fetch extra info like canRefer; admin skips
  const { data: plan2Me } = usePlan2GetMeQuery(undefined, { skip: !isPlan2User });

  // Admin always allowed to refer. Plan 2 user must have canRefer === true.
  const canRefer = isAdminInPlan2 || (isPlan2User && plan2Me?.canRefer);
  const effectiveMemberId = isPlan2User ? (plan2Me?.memberId || memberId) : memberId;
  const effectiveName = isPlan2User ? (plan2Me?.name || authName) : authName;

  const referralLink = canRefer && effectiveMemberId
    ? `${window.location.origin}/plan2/join?ref=${effectiveMemberId}&name=${encodeURIComponent(effectiveName || '')}`
    : null;
  ...
  return (
    <div className="space-y-5 max-w-2xl mx-auto animate-fade-in">
      <div>
        <h1 className="page-heading">Plan 2 Referral</h1>
            ...
            <div className="rounded-2xl p-4" style={{ background: 'rgba(0,102,255,0.08)', border: '1px solid rgba(0,102,255,0.25)' }}>
              <div className="flex items-center gap-2 mb-3">
                ...
                <span className="text-xs font-black uppercase tracking-wider" style={{ color: '#0066ff' }}>
                  {isAdminInPlan2 ? 'Admin Referral Link' : 'Plan 2 Referral'}
                </span>
              </div>
      ...
      <div className="card">
        ...
        <ol className="space-y-3 text-sm t-text-3">
          <li className="flex gap-3">
            ...
            <span>Share your referral link with someone interested in Plan 2.</span>
          </li>
          <li className="flex gap-3">
            ...
            <span>They sign up via your link → get a Plan 2 member ID → verify OTP.</span>
          </li>
          <li className="flex gap-3">
